...
contract ReputationMarket is AccessControl, UUPSUpgradeable, ReentrancyGuard {
  ...
  /**
   * @dev Returns the Ethos Profile contract interface
   */
  ...
  /**
   * @notice Applies and distributes fees from a transaction
   * @param protocolFee The protocol fee amount to apply
   * @param donation The donation amount to apply
   * @param marketOwnerProfileId The profile ID of the market owner
   * @return fees The total fees applied
   */
  function applyFees(
    uint256 protocolFee,
    uint256 donation,
  ...
  /**
   * @notice Calculates fees to be applied to a transaction
   * @param amount The transaction amount to calculate fees for
   * @param isEntry Whether this is an entry (buy) or exit (sell) transaction
   * @return funds The amount after fees
   * @return protocolFee The protocol fee amount
   * @return donation The donation amount
   */
